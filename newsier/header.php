...
<?php _e( 'Skip to content', 'newsier' ); ?></a>
<div class="wrapper" id="custom-background-css">
  ...
  do_action('newsier_action_header_type_section');
  $newspaperup_enable_main_slider = newspaperup_get_option('show_main_banner_section');
  if(is_home() || is_front_page()) {
    if($newspaperup_enable_main_slider){ ?>
      <!--mainfeatured start-->
      <div class="mainfeatured three<?php if (!empty($main_banner_section_background_image)) { echo' over mt-0'; }?>" style="background-image: url('<?php echo esc_attr( get_main_banner_section_background_image_url() ); ?>')">
        <div class="featinner">
          <div class="container">
            <!--row-->  
            <div class="row">
              <?php do_action('newsier_action_front_page_main_section_1'); ?>
            </div><!--/row-->
          </div>
        </div>
      </div>
